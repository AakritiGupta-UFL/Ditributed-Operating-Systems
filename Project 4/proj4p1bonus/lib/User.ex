...
#  def handle_cast({:receive_tweet, userNum, value}, state) do
#    {:ok, receivedTweets} = Map.fetch(state,"receivedTweets")
#    state = get_and_update_in(state.receivedTweets, fn(receivedTweets) ->
#                                            {receivedTweets, put_in(receivedTweets, [userNum], value)}
#                                            end)
#    IO.puts("I received tweet: #{value} tweeted by user: #{userNum}")
#    {:noreply,state}
#  end
  ...
  #  IO.puts("I received tweet: #{value}")
