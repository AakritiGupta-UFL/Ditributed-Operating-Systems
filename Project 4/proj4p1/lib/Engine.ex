...
def printusercount(numUsers) do
Enum.each(0..numUsers-1, fn userNum -> [{userNum,user}]=:ets.lookup(:user_list,userNum)
                              IO.inspect(user)
                            end)
50
end
...
def verifyTweets(numUsers,numTweets,user_pids) do
...
def verifyFollowers(numUsers,sCount,user_pids) do
  ...
  Enum.each(0..numUsers-1, fn user ->   [{_,all_followers}]=:ets.lookup(:followers,Enum.at(user_pids,user))
